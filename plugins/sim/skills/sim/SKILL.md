...
For any Building or Building Blueprint request that asks for an exact folder structure, install path, package tree, installer, or ZIP while the loader/install schema remains unverified, stop before proposing filesystem structure. Do not enumerate or present a concrete install tree — including `Software Inc\Buildings\...`, `Software Inc\Blueprints\...`, `Buildings/*.build`, or `Blueprints/*.xml` — even as an observed, native, likely, provisional, or example layout. If those names appear in evidence, describe them only as storage/export observations and explicitly state that they do not establish installation or delivery semantics. Do not render them as a tree, path mapping, install recipe, or package skeleton. Return `TOOLING_BLOCKED` when the supported native authoring/export path is unavailable. A later refusal to fabricate the final ZIP does not cure an earlier unverified concrete install-tree claim.
